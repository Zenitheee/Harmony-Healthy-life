...
export class HomeStore {
        ...
        DatabaseApi.query(dateToStr(new Date()), (taskList: TaskInfo[], dayInfo: DayInfo) => {
            ...
            DayInfoApi.queryList(weekCalendarInfo.strArr, (res: DayInfo[]) => {
                ...
                for (let i = 0; i < this.dateArr.length; i++) {
                    let tempDayInfo = tempList.find((item: DayInfo) => item.date === this.dateArr[i].dateStr) || new DayInfo(this.dateArr[i].dateStr, 0, 0, false);
    ...
    public getCompletedTasksByDate(date: Date, callback: (completedTasks: number) => void) {
        const dateStr = dateToStr(date);
        DayInfoApi.query(dateStr, (dayInfo: DayInfo) => {
            if (dayInfo) {
                Logger.info('getCompletedTasksByDate', JSON.stringify(dayInfo));
                if (dayInfo.isDone == true) {
                    callback(1);
                }
                else {
                    callback(0);
